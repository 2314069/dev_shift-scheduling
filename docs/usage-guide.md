...
本アプリ（シフトすけっと）を初めて利用する場合は、以下の手順でセットアップを行います。

> **表記について**: UI 上は「店舗」と表示していますが、内部コード・API では `organization` という名称を使用しています。同じものを指しています。

### 1. サインイン（Magic Link）

1. ブラウザで http://localhost:3000 にアクセスします
2. メールアドレスを入力して **「Magic Link を送信」** をクリックします
3. 届いたメールのリンクをクリックするとサインイン完了です

開発環境では SMTP をキャプチャする **Mailpit**（http://localhost:8025）でメールを確認します。

### 2. 店舗の作成（`/onboarding`）

初回サインイン後は自動的に店舗作成画面 `/onboarding` に案内されます。
...
初期設定ガイド画面で以下の順に設定します。

| # | 手順 | 画面 | 目安 |
|---|------|------|------|
| 1 | スタッフを登録する | 設定 (`/settings`) | 必須 |
| 2 | シフト枠を登録する | 設定 (`/settings`) | 必須 |
| 3 | 必要人数を設定する | 設定 (`/settings`) | 任意 |
| 4 | 最初のシフトを作成する | シフト表 (`/schedule`) | — |

スタッフとシフト枠の両方が登録されると、最適化実行が可能になります。

詳細な手順は以下「基本的な流れ」以降のセクションを参照してください。
